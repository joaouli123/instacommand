...
Resolve the Threads Insights HTTP 500 with scope inspection/reconsent verification, then implement Facebook reports and finish Threads per-post/audience coverage. Inspect current official Meta requirements before adding scopes or metric names. Preserve historical data; never guess Facebook Page mappings based on profile names. Deploy the new exact-link validation and inspect live Page associations without publishing test content.
