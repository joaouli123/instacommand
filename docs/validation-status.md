...
Complete report exports and Facebook reach/views/audience coverage; resolve the Threads Insights HTTP 500 with scope inspection/reconsent verification, and finish Threads per-post/audience coverage. The broad AI/editorial/upload/publishing requirements remain open. AI code inspection confirms only text modes caption/plan/audit/reply; image input, artwork generation and three-post-to-drafts workflow are not yet present. Inspect current official Meta requirements before adding scopes or metric names. Preserve historical data; never guess Facebook Page mappings based on profile names.
