...
type Dashboard = { followers: number; followerGrowth: number; reach: number; impressions: number; interactions: number; pendingPosts: number }
type TimelineItem = { date: string; likes: number; comments: number; saves: number; shares: number; reach: number; impressions: number; engagement: number; posts: number; interactions?: number }
...
export default function AnalyticsPage() {
  ...
  const engagementData = useMemo(() => engagement.map((item) => ({ name: formatDate(item.date), interacoes: item.interactions ?? item.likes + item.comments + item.saves + item.shares, taxa: item.engagement })), [engagement])
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <Tabs defaultValue="visao"><TabsList className="mb-6"><TabsTrigger value="visao">Visão Geral</TabsTrigger><TabsTrigger value="posts">Conteúdo</TabsTrigger><TabsTrigger value="audiencia">Audiência</TabsTrigger></TabsList>
      <TabsContent value="visao" className="space-y-6"><div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">{[
        ...
        { label: "Interações", value: dashboard?.interactions ? formatNumber(dashboard.interactions) : "Indisponível", sub: dashboard?.interactions ? "Likes, comentários, salvos e compartilhamentos" : "Nenhuma interação importada", icon: Heart },
