...
export function DailyContentPlan({ accountId, topic, audience, tone, objective, onEdit }: Props) {
  ...
  const [saved, setSaved] = useState<Array<{ id: string }>>([])
  ...
  const save = async () => {
    if (!plan || saving || saved.length) return
    const version = request.current
    setSaving(true)
    try {
      const response = await api.saveDailyDrafts({ requestId, accountId, date, plan }) as { posts: Array<{ id: string }> }
      if (version !== request.current) return
      setSaved(response.posts); toast.success('Três rascunhos salvos. Nenhum post foi agendado.')
