...
export function StatsCards({ stats, loading, error, onRetry }: { stats?: DashboardStats; loading?: boolean; error?: boolean; onRetry?: () => void }) {
  ...
  return <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4">
    {cards.map((stat) => <Card key={stat.title} className="min-w-0 rounded-2xl border border-slate-200/80 bg-white p-3.5 transition-all hover:border-indigo-200 hover:shadow-md sm:p-5">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-1.5 sm:mb-3 sm:flex-nowrap sm:gap-3"><div className={`rounded-lg p-2 sm:rounded-xl sm:p-2.5 ${stat.iconBg}`}><stat.icon size={18} strokeWidth={2.2} /></div><span className="inline-flex max-w-full items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[9px] font-semibold leading-3 text-slate-500 sm:text-[11px]">{stat.title === "Seguidores" && stats?.hasFollowerHistory && stats.followerGrowth !== 0 ? <TrendingUp size={12} /> : <Minus size={12} />}{stat.trend}</span></div>
