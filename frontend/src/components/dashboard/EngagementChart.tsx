...
export function EngagementChart({ data, loading = false, error = false, onRetry }: { data: Point[]; loading?: boolean; error?: boolean; onRetry?: () => void }) {
  ...
  return <Card className="flex h-[300px] flex-col rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xs sm:h-[340px] lg:h-[380px]">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2 sm:mb-5"><div><h3 className="text-sm font-bold text-slate-900 sm:text-base">Interações e engajamento</h3><p className="text-[11px] font-medium text-slate-500 sm:text-xs">Somente métricas reais das publicações importadas</p></div>{chartData.length > 0 && <div className="flex items-center gap-1 rounded-full border border-sky-100 bg-sky-50 px-2.5 py-1 text-[10px] font-semibold text-sky-700 sm:text-xs"><Activity size={13}/>{averageInteractions.toLocaleString("pt-BR", { maximumFractionDigits: 0 })} médias{hasRate ? ` · ${averageRate.toLocaleString("pt-BR", { maximumFractionDigits: 2 })}% ER` : ""}</div>}</div>
